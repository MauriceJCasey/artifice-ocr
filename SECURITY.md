...
Only the most recent release receives security updates. The current version
is listed in `apps/artifice-ocr/pyproject.toml`.
...
> [**Report a vulnerability**](https://github.com/MauriceJCasey/artifice-ocr/security/advisories/new)
> — or from the repository, go to **Security → Advisories → Report a vulnerability**.

Please include enough detail to reproduce the issue: the version, and a
